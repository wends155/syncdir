...
* **2026-07-14**: Builder fixed real-time deletion bug where destination files were deleted directly regardless of `propagate_deletions = false` config, and optimized SQLite block signature persistence inside `save_file` by wrapping inserts in an explicit transaction and converting `SqliteHashStore` to use a `Mutex` to satisfy Send + Sync traits. Verified with 17 passing tests and zero clippy/fmt errors.
