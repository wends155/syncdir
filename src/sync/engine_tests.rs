...
fn test_sync_file_small_file_verify_writes() {
    ...
    let target_cfg =
        TargetSyncConfig::from_config(&config, TargetDir::from_validated(dst.clone())).unwrap();
    let engine = LocalSyncEngine::new(MockHashStore::new(), target_cfg.with_verify_writes(true));
...
fn test_delta_sync_large_file() {
    ...
    f.set_times(
        fs::FileTimes::new().set_modified(SystemTime::now() + std::time::Duration::from_secs(5)),
    )
    .unwrap();
