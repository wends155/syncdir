...
└── src/
    ...
    │   ├── mod.rs        # Central facade and public re-exports (completely decoupled from storage)
    │   ├── builder.rs    # ConfigBuilder and TargetSyncConfigBuilder (with #[must_use])
    ...
    │   ├── engine.rs     # Core LocalSyncEngine orchestration coordinator (<800 LOC)
    │   ├── engine_tests.rs # Unit tests for LocalSyncEngine coordination and edge cases
    ...
    │   ├── traits.rs     # Pure role traits and engine abstractions (SyncEngine, FileSynchronizer, etc.)
    │   ├── types.rs      # Standalone FileSyncTask and safe timestamp normalization helpers
    │   └── worker/       # Decomposed sync worker subsystem (<800 LOC per submodule)
    │       ├── mod.rs    # Subsystem facade and worker thread entry point (start_sync_worker)
    │       ├── queue.rs  # Dual min-heap DebounceQueue with deduplication and compaction
    │       ├── reachability.rs # ReachabilityMonitor managing destination online/offline state
    │       ├── state.rs  # SyncWorkerState, bounded FailureTracker (5k cap), exponential backoff
...
### `db`
* **Owns**: Connection management to isolated local SQLite databases with WAL mode (`PRAGMA journal_mode = WAL`, `PRAGMA synchronous = NORMAL`, `foreign_keys = ON`), composite index `idx_block_hashes_file_block` on `(file_id, block_index)`, unique index `idx_file_metadata_relative_path`, prepared statement caching (`prepare_cached`), schema versioning (`db_version = "4"`), recording and retrieving file metadata (`FileRecord` strictly encapsulated with private fields, `relative_path: RelativePath`, `file_size: u64`, and getter accessors) and fixed 32-byte block digests (`BlockHash = [u8; 32]`), path-keyed block hash lookups, strongly-typed file enumeration via `HashStore::list_files(&self) -> Result<Vec<RelativePath>, SyncError>`, single-query bulk metadata preload returning a flat vector (`HashStore::list_all_records(&self) -> Result<Vec<FileRecord>, SyncError>`), atomic file and directory deletion wrapped in a single SQLite transaction with rollback via `delete_file` (delegating to `delete_files_batch(&[path])`), single-block UPSERT updates with `RETURNING id` (`save_file`), in-place path key sanitization (`.drain()`), exact Unicode path key preservation (preserving superscript and multi-byte UTF-8 without lossy normalization), and safe directory deletion via exact prefix matching `substr(relative_path, 1, length(?1) + 1) = ?1 || '/'`. Generates separate cache database files (`sigcache_<hash>.db`) named using the Blake3 hash of the target path to prevent collisions via `SqliteHashStore::cache_db_path`. Decoupled from `config`: `StoreConfig` is a pure configuration value object constructed via `StoreConfig::new(block_size, threshold)` with zero imports of `config.rs`. `HashStore` is implemented for `Arc<S>` and `&S` for zero-copy multi-threaded sharing.
* **Internal Decomposition**:
  * `db::traits`: Pure storage domain models (`FileRecord`, `StoreConfig`, `BlockHash`), key normalization (`path_to_sqlite_key`), and `HashStore` trait definition with zero `rusqlite` dependencies.
  * `db::sqlite`: Concrete SQLite persistence engine (`SqliteHashStore`), connection lifecycle, WAL pragma setup, prepared statement caching, and atomic single-transaction deletions.
  * `db::mock`: In-memory test double (`MockHashStore`, `MockStoreInner`, `MockStoreErrorHook`) with operation counters for testing without disk I/O.
  * `db::mod`: Module facade exposing public contracts without wildcard re-exports.
...
* **Collaborating Components (Decomposed Engine)**:
  ...
  * `sync::delta::DeltaTransferEngine<S: HashStore>`: Standalone leaf engine for in-place delta synchronization, chunked file reading, Blake3 block hashing with preallocated hash vectors, dirty range pooling, and block collection bypass under `MetadataAndFlush`.
  * `sync::archive::ArchiveManager`: Standalone leaf engine for retention-based archive subfolder management, timestamped backups, root junction verification, ancestor safety checks before directory creation, TOCTOU symlink checks before removal, safe directory pruning, and `ReparseCache` integration in `archive_dest_file_only`.
  * `sync::scanner::DirectoryScanner`: Standalone leaf engine for directory traversal, inspecting `entry.file_type()` before allocating `entry.path()`, single path allocation per entry, batch DB record saving, case-insensitive deletion detection, cancellation, and safety threshold checks.
  * `sync::engine::LocalSyncEngine<S: HashStore>`: Central coordinator composing collaborating leaf transfer engines, maintaining sub-800 LOC ceiling (<750 LOC) by extracting pure traits to `traits.rs` and unit tests to `engine_tests.rs`. Implements segregated role traits (`FileSynchronizer`, `FileDeleter`, `BatchFlusher`, `ScanEngine`, `ArchiveEngine`, `CasingAligner`, `ArchiveManager`) and `FullScanDriver`.
  * `sync::engine_tests`: Standalone child test module (`1,600+` lines) maintaining comprehensive regression coverage for composed sync engine lifecycles, SMB timestamp tolerance, destination truncation recovery, and fast-path casing alignment bypass.
  * `sync::path_safety`: Win32 reparse point validation, ancestor junction guards, two-phase non-blocking cache verification, zero-allocation peekable component iterator traversal, read-lock probe in `ReparseCache::evict_dir`, non-existent root cache bypass, and path traversal defenses.
  * `sync::worker`: Discrete worker state machine subsystem decomposed into modular files under `src/sync/worker/` (`mod.rs`, `queue.rs`, `reachability.rs`, `state.rs`, `context.rs`, `runner.rs`, `tests.rs`). Enforces bounded failure tracking (5,000 capacity FIFO eviction), catch-up scan flag clearing invariants on `TriggerFullScan` and destination reconnect, and dependency injection of `Arc<dyn NetworkResolver>` in `SyncWorkerContextBuilder`.
  * `sync::mock`: Thread-safe mock implementation (`MockSyncEngine` and `MockSyncStatusObserver`) with zero-panic lock acquisitions for unit and integration testing.
* **Encapsulation**: All submodules are encapsulated via `pub(crate) mod`. The crate-level facade `syncdir::sync` exposes `SyncEngine`, `FileSynchronizer`, `FileDeleter`, `BatchFlusher`, `ScanEngine`, `ArchiveEngine`, `FullScanDriver`, `LocalSyncEngine`, `SyncCommand`, `ScanOutcome`, `ConnectivityState`, `WatcherState`, `SyncWorkerContext`, `SyncWorkerContextBuilder`, `start_sync_worker`, `FileSyncTask`, `FileSyncTaskBuilder`, `ReparseCache`, `MockSyncEngine`, and `MockSyncStatusObserver`.
...
* **Mock Availability**:
  ...
  * `MockFullScanDriver` (scoped to `src/sync/full_scan.rs` tests): In-memory driver for isolating `FullScanCoordinator` pipeline tests.
...
> **Cycle Resolution Note**: The historical circular dependency between `config` and `net` is permanently resolved by extracting path canonicalization into `path_util`. `db` is completely decoupled from `config`, importing `path_util` (for `RelativePath`) and `error` only. `daemon` is decoupled from `tray` and `startup`, delegating UI/startup dispatching to `main`. `main` acts as the composition root, decoupled from `winit`, consuming `Arc<dyn SyncStatusObserver>` from `tray`.
...
* **Comprehensive In-Memory Mocks**: Six isolated mock implementations providing 100% test isolation, re-exported under `syncdir::test_support`:
  ...
  * `MockFullScanDriver` (`src/sync/full_scan.rs` tests): In-memory test double isolating `FullScanCoordinator` pipeline execution without filesystem or SQLite side-effects.
