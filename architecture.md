...
* **Collaborating Components (Decomposed Engine)**:
  * `sync::types`: Standalone leaf module containing `FileSyncTask` and timestamp normalization helpers (`safe_epoch_duration_millis`, `safe_modified_millis`), completely eliminating inverted circular dependencies from leaf transfer engines (`delta.rs`, `small_file.rs`) to `engine.rs`.
  * `sync::full_scan::FullScanCoordinator`: Standalone decomposed pipeline coordinator for full directory synchronization with zero-allocation lookups on Windows (`NormalizedCaseFoldedPath`), separating scan stages into dedicated, low-complexity methods (`resolve_active_destination`, `collect_source_files`, `load_cached_records`, `synchronize_files`, `reconcile_deletions`, `prune_archive`).
  ...
  * `sync::engine::LocalSyncEngine<S: HashStore>`: Central coordinator composing the collaborating leaf transfer engines, implementing `SyncEngine` by coordination. All coordination methods (`run_cancellable_full_scan_impl`, `flush_record_batch`, `delete_file_from_dest`, `prune_destination_archive`, `archive_dest_file_only`, `sync_delta_large_file_core`, `sync_small_file_core`) are consolidated directly in `engine.rs`.
...
* **Encapsulation**: All submodules are encapsulated via `pub(crate) mod`. The crate-level facade `syncdir::sync` exposes `SyncEngine`, `LocalSyncEngine`, `SyncCommand`, `ScanOutcome`, `ConnectivityState`, `WatcherState`, `SyncWorkerContext`, `SyncWorkerContextBuilder`, `start_sync_worker`, `FileSyncTask`, and `MockSyncEngine`.
